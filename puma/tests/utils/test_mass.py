...
class VertexMassTestCase(unittest.TestCase):
    ...
    def test_multiple_vertices(self):
        ...
        vtx_mass_1 = np.sqrt((e[0, 0] + e[0, 1])**2 - (px[0, 0] + px[0, 1])**2 - (py[0, 0] + py[0, 1])**2 - (pz[0, 0] + pz[0, 1])**2)
        vtx_mass_2 = np.sqrt((e[0, 2] + e[0, 3] + e[0, 4])**2 - (px[0, 2] + px[0, 3] + px[0, 4])**2 - (py[0, 2] + py[0, 3] + py[0, 4])**2 - (pz[0, 2] + pz[0, 3] + pz[0, 4])**2)

        vtx_masses = calculate_vertex_mass(track_pt, track_eta, track_phi, vtx_idx, particle_mass=0.13957)
