...
def LoadDataset(file_path,kinematic_cuts,  n_jets=-1, n_tracks=40):
    ...
    event_var = ["primaryVertexToBeamDisplacementX", "primaryVertexToBeamDisplacementY", "primaryVertexToBeamDisplacementZ", "primaryVertexToTruthVertexDisplacementX", "primaryV\
ertexToTruthVertexDisplacementY", "primaryVertexToTruthVertexDisplacementZ"]
...
def ExtractHadronInfo(f, good_jets):
    ...
    ordered_hadron_indices = GetOrderedHadrons(hadrons["barcode"], hadrons["ftagTruthParentBarcode"], n_max_showers=2)
    ...
    hadron_mask_first_shower = np.transpose(np.where(hadron_indices_first_shower >= 0, 1, 0)) #(n_hadrons, n_jets)  
    
    # Associate Tracks to Hadrons
    truth_SV_finding_exclusive, truth_SV_finding_first_shower, truth_SV_finding_inclusive = AssociateTracksToHadron(tracks["ftagTruthParentBarcode"], hadrons["barcode"], hadron_mask_first_shower)
    
    # Now re-order the exclusive ones using the hadron index
    tracks_associated_to_sv1 = select_tracks(truth_SV_finding_exclusive, hadron_indices_first_shower, element=0)
    n_tracks_sv1 = np.sum(tracks_associated_to_sv1, axis=1)
    
    tracks_associated_to_sv2 = select_tracks(truth_SV_finding_exclusive, hadron_indices_first_shower, element=1)
    n_tracks_sv2 = np.sum(tracks_associated_to_sv2, axis=1)
    
    tracks_associated_to_sv3 = select_tracks(truth_SV_finding_exclusive, hadron_indices_first_shower, element=2)
    ...
    Exclusive_SV_1_2_3 = ( (sv_1, n_tracks_sv1, tracks_associated_to_sv1), (sv_2, n_tracks_sv2, tracks_associated_to_sv2), (sv_3, n_tracks_sv3, tracks_associated_to_sv3)) 
...
(sv1, sv2, sv3), sv1_shower, inclusive_vertex = ExtractHadronInfo(dataset, good_jets) # process truth Hadron information (you will need it later)
...
for flavour in ["bjets", "cjets"]:
    ...
    h_tracks = Histogram(sum_track_pt[selection], label="$\\sum$ SV Hadron tracks (Reco)", histtype="step", alpha=1)
    ...
    h_hadron = Histogram(truth_pt[selection], label="Hadron (Truth)", histtype="step", alpha=1)
    h_tracks = Histogram(GN2_sum_track_pt[selection], label="$\\sum$ tracks (GN2 SV)", histtype="step", alpha=1)
